...
export function BirthDatePicker({ value, onChange }: { value?: string; onChange: (date: string) => void }) {
  ...
  return <View style={{ gap: 18 }}>
    ...
    {Platform.OS === "ios" || open ? <DateTimePicker value={date} mode="date" display={Platform.OS === "ios" ? "spinner" : "default"} maximumDate={today} minimumDate={new Date(today.getFullYear() - 120, today.getMonth(), today.getDate())} themeVariant={c.dark ? "dark" : "light"} locale={language} onValueChange={(_event, selected) => { setOpen(false); onChange(birthDateString(selected)); }} onDismiss={() => setOpen(false)} /> : null}
