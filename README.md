...
The interface now uses the supplied outline/filled navigation icons, a compact header and a Monday-first cooking streak panel. Profile includes account editing, community recipe discovery and an honest Plus beta preview. Settings add searchable country/product-region selection, profile visibility controls and a Notifications switch. Password fields include visibility controls and the shared form layout adjusts for the iOS keyboard.

Supported iOS 26 devices use native Liquid Glass surfaces for navigation and shared buttons, with a solid fallback for unsupported devices, Android, web and Reduce Transparency. Native appearance and keyboard behavior still require physical-device verification. Profile visibility choices are saved locally and default to hidden; public profiles and community publishing are not yet released. The verification badge means email verified. Future three-day subscription trials are reserved for the yearly plan only.

Native onboarding now includes an optional notification permission page and a five-second test alert, also accessible through Settings. Guided cooking timers schedule local notifications when permission is granted and notifications are enabled. Canceling a timer, finishing cooking or leaving its cooking screen cancels the pending alert. Local notifications are supported in Expo Go; permission and notification branding there belong to Expo Go. Browser timers remain foreground-only. Physical iPhone delivery validation is pending, and device notification/Focus settings can affect delivery. Paid subscriptions, discount offers and store review prompts remain deferred until an appropriate native build and store setup are available.
