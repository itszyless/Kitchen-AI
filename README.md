...
Native onboarding now includes an optional notification permission page and a five-second test alert, also accessible from Profile. Guided cooking timers schedule local notifications when permission is granted. Canceling a timer, finishing cooking or leaving its cooking screen cancels the pending alert. Local notifications are supported in Expo Go; permission and notification branding there belong to Expo Go. Browser timers remain foreground-only. Physical iPhone delivery validation is pending, and device notification/Focus settings can affect delivery. Paid subscriptions, discount offers and store review prompts remain deferred until an appropriate native build and store setup are available.
